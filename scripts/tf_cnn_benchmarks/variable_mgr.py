...
class VariableMgrDistributedReplicated(VariableMgr):
  ...
  def append_apply_gradients_ops(self, gradient_state, opt, grads, training_ops):
    ...
    gradient_recv_device = None
    dependencies = []
    if self.benchmark_cnn.offload_enabled:
      gradient_recv_device = self.benchmark_cnn.offload_devices[
        self.benchmark_cnn.group_index]
    else:
      gradient_recv_device = self.benchmark_cnn.param_server_device
    ...
    for i, (g, v) in enumerate(grads):
      if self.benchmark_cnn.offload_enabled:
        ...
        with tf.device(gradient_recv_device):
          ...
          with tf.control_dependencies([agg_barrier]):
            if self.benchmark_cnn.node_index==0:
              apply_op = opt.apply_gradients(
                [(offload_gradients[self.benchmark_cnn.group_index].read_value(), v)]
              )
            else:
              apply_op = tf.no_op()
          ...
          with tf.control_dependencies([apply_op]):
            if self.benchmark_cnn.node_index==0:
              ...
              reset_op = tf.no_op()
        dependencies.append(reset_op)
      else: # not using offload
        with tf.device(gradient_recv_device):
          apply_gradients_op = opt.apply_gradients([(g,v)])
        dependencies.append(apply_gradients_op)
      ...
      barrier = self.benchmark_cnn.add_sync_queues_and_barrier(
          'replicate_variable_%s' % i, dependencies)
